...
const MyTimeline = () => {
  ...
  const handleClick = (e, i) => {
    ...
    if (carouselRef.current) {
      const scrollLeft = Math.floor(
        carouselRef.current.scrollWidth * 0.7 * (i / EducationDetails.length)
      );
  ...
  return (
    <Section id="my-timeline">
      <SectionDivider />
      <SectionTitle main>My Timeline</SectionTitle>
      {TOTAL_ABOUT_ME_TEXT_LENGTH > 300 ? (
        <SectionText>
          {aboutMeText[0]} ...
          {<Span onClick={() => setShowSummary(true)}>Read More</Span>}
        </SectionText>
      ) : (
